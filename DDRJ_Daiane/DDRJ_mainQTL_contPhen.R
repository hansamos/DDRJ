...
#
DDRJ_mainQTL_notdensemap<-function(dados,loc.marc=seq(from=0,to=(ncol(dados)-2),by=1),directory=getwd(),finsample=1000,burnin=1000,jumps=1, neta.a=0.1,neta.b=0.1,media.mi=0,sigma2.mi=100,sigma2.alpha=100,media.alpha=0,sigma2.delta=100,media.delta=0
){
	...
	# loc.marc = a vector with the position (in Morgans) of each marker and the first element should be equal to zero. If it is not available, the algorithm assumes 1 M between the markers which represents almost no association between the markers' genotype;
	...
	# finsample = MCMC sample size after burnin and jumps. The default is assumed as 1000;
	# burnin = number of iterations that will be discarded in the beginning of the MCMC. The default is assumed as 1000;
...
,append=T)
#		cat('',round(probacemerge,2),file=paste(directory,"/prob_accep_merge.txt",sep="")
#,append=T)
		#
...
set.seed(100)
DDRJ_mainQTL_notdensemap(dados=dados,loc.marc=loc.marc,directory=directory)
#
...
mat.coef<-matrix(0,nrow=finsample,ncol=(max(num.qtls)*2+1))
obs<-1
obs2<-1
